...
export default function SetupPage() {
  ...
  const [step, setStep] = useState<"input" | "analysing" | "generating">(
    "input",
  );
  ...
  async function handleResumeUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setResumeFile(null);
      setResumeText("");
      setError("Failed to parse resume. Please try again with a valid PDF.");
      console.error(err);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white px-4 py-12">
      ...
      <div className="fixed top-0 left-[50%] translate-x-[-50%] w-125 h-100 bg-violet-600/10 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="relative z-10 max-w-2xl mx-auto">
        ...
        <p className="text-gray-500 text-sm mb-10">
          Paste the job description and upload your resume. We'll generate 8
          questions tailored to that exact role.
        </p>
        ...
        <div className="mb-6">
          ...
          <textarea
            ...
            rows={10}
            disabled={isLoading}
            className="w-full bg-white/3 border border-white/8 rounded-xl px-4 py-3 text-sm text-gray-300 placeholder-gray-600 resize-none focus:outline-none focus:border-violet-500/50 transition-colors disabled:opacity-50"
          />
        ...
        <div className="mb-8">
          ...
          <label
            className={`flex items-center gap-4 p-4 bg-white/3 border border-dashed rounded-xl cursor-pointer transition-colors ${resumeFile ? "border-violet-500/40" : "border-white/8 hover:border-white/20"} ${isLoading ? "opacity-50 pointer-events-none" : ""}`}
          >
        ...
        {isLoading && (
          <div className="mb-6 px-4 py-3 bg-violet-500/10 border border-violet-500/20 rounded-lg text-sm text-violet-400 flex items-center gap-3">
            ...
            {step === "analysing"
              ? "Analysing job description and resume..."
              : "Generating your personalised questions..."}
          </div>
        )}
