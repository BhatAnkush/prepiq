...
import {
  Zap, Clock, ChevronRight, CheckCircle2, XCircle,
  AlertTriangle, Loader2, BarChart3, ArrowRight, Star
} from "lucide-react"
...
export default function InterviewPage() {
  ...
  const [phase, setPhase] = useState<"answering" | "streaming" | "scored" | "done">("answering")
  ...
  useEffect(() => {
    fetch(`/api/session?id=${sessionId}`)
      ...
      .then((data) => {
        setSession(data)
        setConversationHistory([{ role: "assistant", content: data.questions[0]?.questionText }])
      })
  }, [sessionId])
  ...
  async function handleSubmit() {
    ...
    const res = await fetch("/api/chat", {
      ...
      body: JSON.stringify({ sessionId, questionId: currentQuestion.id, userAnswer, conversationHistory: newHistory }),
    })
    ...
    const scoreRes = await fetch("/api/score", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sessionId, questionId: currentQuestion.id, userAnswer, questionText: currentQuestion.questionText, roleTitle: session.roleTitle, techStack: session.analysis.techStack }),
    })
  ...
  function handleNext() {
    if (!session) return
    if (currentIdx + 1 >= session.questions.length) { setPhase("done"); return }
    const nextIdx = currentIdx + 1
    setCurrentIdx(nextIdx)
    setUserAnswer("")
    setInterviewerResponse("")
    setConversationHistory([{ role: "assistant", content: session.questions[nextIdx].questionText }])
    setPhase("answering")
    ...
    return (
      <main className="min-h-screen bg-[#0a0a0a] text-white px-4 py-12">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <h1 className="text-3xl font-black tracking-tight mb-2">Interview complete</h1>
          <p className="text-gray-400 text-sm mb-8">{session.companyName} · {session.roleTitle}</p>
          ...
          <div className="flex gap-3 justify-center">
            <button onClick={() => router.push("/dashboard")} className="flex items-center gap-2 px-5 py-2.5 bg-violet-600 hover:bg-violet-500 rounded-lg text-sm font-medium transition-colors">
              View full report <ArrowRight className="w-4 h-4" />
            </button>
            <button onClick={() => router.push("/setup")} className="px-5 py-2.5 border border-white/10 hover:border-white/20 rounded-lg text-sm text-gray-400 hover:text-white transition-colors">
              New interview
            </button>
          </div>
        </div>
  ...
  const timerColor = timeLeft > 60 ? "#8b5cf6" : timeLeft > 30 ? "#f59e0b" : "#ef4444"

  const difficultyConfig: Record<string, { color: string; icon: JSX.Element }> = {
    hard: { color: "text-red-400 bg-red-500/10 border-red-500/20", icon: <AlertTriangle className="w-3 h-3" /> },
    medium: { color: "text-amber-400 bg-amber-500/10 border-amber-500/20", icon: <ChevronRight className="w-3 h-3" /> },
    easy: { color: "text-green-400 bg-green-500/10 border-green-500/20", icon: <CheckCircle2 className="w-3 h-3" /> },
  }
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="border-b border-white/[0.06] px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-400">{session.companyName} · {session.roleTitle}</span>
        </div>
        <div className="flex items-center gap-4">
          <span className="text-xs text-gray-500">{currentIdx + 1} / {session.questions.length}</span>
      ...
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className={`flex items-center gap-1 px-2 py-0.5 rounded text-xs border ${difficultyConfig[currentQuestion.difficulty]?.color}`}>
            {difficultyConfig[currentQuestion.difficulty]?.icon}
            {currentQuestion.difficulty}
          </span>
          <span className="text-xs text-gray-600">↳ {currentQuestion.targetSkill}</span>
        </div>

        {/* Question */}
        <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-6 mb-6">
          <p className="text-base leading-relaxed font-medium">{currentQuestion.questionText}</p>
        </div>
        ...
        {(phase === "answering" || phase === "streaming") && (
          <>
            ...
            />
            <button onClick={handleSubmit} disabled={!userAnswer.trim() || phase === "streaming"}
        ...
        {phase === "streaming" && interviewerResponse && (
          <div className="mt-6 p-4 bg-white/[0.03] border border-white/[0.06] rounded-xl">
            <p className="text-xs text-gray-500 mb-2 font-medium">Interviewer</p>
            <p className="text-sm text-gray-300 leading-relaxed">{interviewerResponse}</p>
          </div>
        )}
          ...
          <div className="space-y-4">
            {interviewerResponse && (
              <div className="p-4 bg-white/[0.03] border border-white/[0.06] rounded-xl">
                <p className="text-xs text-gray-500 mb-2 font-medium">Interviewer follow-up</p>
                <p className="text-sm text-gray-300 leading-relaxed">{interviewerResponse}</p>
              </div>
            )}
            ...
            <div className="p-5 bg-white/[0.03] border border-white/[0.08] rounded-xl">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`text-2xl font-black ${currentScore.score >= 8 ? "text-green-400" : currentScore.score >= 5 ? "text-amber-400" : "text-red-400"}`}>
                  {currentScore.score}/10
                </span>
              </div>
              <p className="text-xs text-gray-400 mb-4 leading-relaxed">{currentScore.feedback}</p>

              {currentScore.strengths?.length > 0 && (
                <div className="mb-3">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <CheckCircle2 className="w-3.5 h-3.5 text-green-400" />
                    <p className="text-xs font-semibold text-green-400">What worked</p>
                  </div>
                  {currentScore.strengths.map((s: string, i: number) => (
                    <p key={i} className="text-xs text-gray-400 mb-1 pl-5">{s}</p>
                  ))}
                </div>
              )}
              ...
              {currentScore.improvements?.length > 0 && (
                <div className="mb-4">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <p className="text-xs font-semibold text-amber-400">Improve on</p>
                  </div>
              ...
              <div className="border-t border-white/[0.06] pt-4">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <p className="text-xs font-semibold text-violet-400">Model answer (9/10)</p>
                </div>
                <p className="text-xs text-gray-400 leading-relaxed">{currentScore.modelAnswer}</p>
              </div>
            </div>
            ...
            <button onClick={handleNext}
              ...
              {currentIdx + 1 >= session.questions.length
                ...
                : <>Next question ({currentIdx + 2}/{session.questions.length}) <ChevronRight className="w-4 h-4" /></>
              }
            </button>
          </div>
        )}
